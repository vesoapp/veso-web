...
let ampHtml;if(serverless){req.url+=(req.url.includes('?')?'&':'?')+'amp=1';ampHtml=(await renderMethod(req,res,'export')).html;}else{ampHtml=await renderMethod(req,res,page,{...query,amp:1},curRenderOpts);}if(!curRenderOpts.ampSkipValidation){await validateAmp(ampHtml,page+'?amp=1');}await mkdir(ampBaseDir,{recursive:true});await writeFileP(ampHtmlFilepath,ampHtml,'utf8');}}if(curRenderOpts.pageData){const dataFile=(0,_path.join)(pagesDataDir,htmlFilename.replace(/\.html$/,'.json'));await mkdir((0,_path.dirname)(dataFile),{recursive:true});await writeFileP(dataFile,JSON.stringify(curRenderOpts.pageData),'utf8');}results.fromBuildExportRevalidate=curRenderOpts.revalidate;await writeFileP(htmlFilepath,html,'utf8');return results;}catch(error){console.error(`\nError occurred prerendering page "${path}". Read more: https://err.sh/next.js/prerender-error:\n`+error);return{...results,error:true};}}