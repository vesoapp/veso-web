...
class Server {
    constructor({ dir = '.', staticMarkup = false, quiet = false, conf = null, dev = false, customServer = true, } = {}) {
        ...
        const env = load_env_config_1.loadEnvConfig(this.dir, dev);
    ...
    async handleApiRequest(req, res, pathname, query) {
        ...
        await api_utils_1.apiResolver(req, res, query, pageModule, this.renderOpts.previewProps, this.renderOpts.env, this.onErrorMiddleware);
        return true;
    }
    ...
    async renderToHTMLWithComponents(req, res, pathname, { components, query }, opts) {
        ...
        const urlPathname = `${url_1.parse(req.url || '').pathname}${query.amp ? '.amp' : ''}`;
